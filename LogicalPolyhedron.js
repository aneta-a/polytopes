...
//---------Platonic Solids-----------------------------
var tetrahedron = new LogicalPolyhedron(
...
var cube = new LogicalPolyhedron( cubeVertices, cubeFaces);
var octahedron = cube.getDual();

var gr = Utils.goldenRatio;
var gr1 = gr + 1;

var dodeVertices = cube.vertices.slice();
for (var k = 0; k < 3; k++){
...
for (var i = 0; i < 12; i++) {
	var c = new THREE.Vector3().addVectors(cube.vertices[cube.edges[i][0]], cube.vertices[cube.edges[i][1]]);
	var k0 = -1;
	for (var k = 0; k < 3; k++) {
		if (Math.abs(c.getComponent(k)) < 1e-10) k0 = k;
	}
	var m = (2*c.getComponent((k0+1)%3)+c.getComponent((k0+2)%3)+6)/4;
	dodeFaces[i] = [cube.edges[i][0], 8+4*k0+m, cube.edges[i][1], 8+4*((k0+1)%3)+2*(m%2), 8+4*((k0+1)%3)+2*(m%2)+1]

}

var dodecahedron = new LogicalPolyhedron(dodeVertices, dodeFaces);
var icosahedron = dodecahedron.getDual();
